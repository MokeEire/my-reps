...
To understand how our representatives are acting, we need to know what they're actually doing.
This can come in the form of introducing legislation, being part of a committee that considers a bill, and simply voting for or against legislation at various stages in the process.
The US government maintains the govinfo API which provides access to a lot of congressional information, including the actions taken on a bill throughout its lifetime, which is the first source I'll be looking at to explore representative behaviour.
...
```{r}
add_count(house_117_df, billNumber, name = "rows_per_bill") %>% 
  count(rows_per_bill)
```

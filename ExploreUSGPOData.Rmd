...
```{r}
sample_package = sample_n(bill_status$packages$packageId, 1)
...
```{r}
map(bill_status$packages$packageId, getPackages)
...
One bill number per row?

```{r}
add_count(hr117_df, billNumber) %>% 
  count(n)
```

...
(hr117_counts = hr117_df %>% 
  mutate(across(where(is_list), function(col) case_when(map_lgl(col, is_tibble) ~ col,
                                T ~ list(tibble()))),
...
hr117_df %>% 
  ...
  summarise(bills = n(), across(where(is_list), ~sum(map_dbl(., is_tibble), na.rm=T))) %>% 
  # Pivot and calculate this as a proportion
  pivot_longer(cols = c(bills:cosponsors), names_to = "element", values_to = "count", ) %>% 
  group_by(billType) %>% 
...
```{r}
map(hr117_df$committees, nrow) %>% glimpse()
...
```{r}
glimpse(hr117_counts)
```

Confirm that there is one bill number per row?

```{r}
add_count(hr117_counts, billNumber, name = "rows_per_bill") %>% 
  count(rows_per_bill)
```
...
hr117_counts %>% 
  rowwise() %>% 
  mutate(first_action = min(actions$actionDate, na.rm=T)) %>% 
  filter(first_action!=introducedDate) %>% 
  select(billNumber, introducedDate, first_action, actions) %>% 
  unnest(actions)
```

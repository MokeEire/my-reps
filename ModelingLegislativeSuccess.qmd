...
Why do some bills become law while others are merely a drop in the vast archives of Congress? Presumably the bills that become law address important problems of the time, are well written, and legislators expect them to be effective.
...
(bills_became_law = all_bills %>% 
    ...
    mutate(became_law = map_lgl(actions, ~ "BecameLaw" %in% .$type)) %>% 
...
```{r}
bills_became_law %>% 
  # Count instances of each title
  count(title) %>% 
  # Keep titles with more than 1
  filter(n > 1)
```

What types of legislation became law?
...
Which policy areas were the bills related to?

```{r}
(new_laws_policy_areas = bills_became_law %>% 
  # Expand policy areas
  unnest(policy_areas, keep_empty = T) %>% 
   mutate(policy_areas = fct_infreq(fct_explicit_na(policy_areas))) %>% 
  # Count instances of each
  count(policy_areas))
```

Top/Bottom 10 policy areas?

```{r}
(top_policy_areas = new_laws_policy_areas %>% 
  slice_max(n, n = 10, with_ties = F) %>% 
   mutate(policy_areas = fct_rev(policy_areas)))

(bottom_policy_areas = new_laws_policy_areas %>% 
  slice_min(n, n = 10, with_ties = F))
```

```{r}
library(patchwork)

top_plot = ggplot(top_policy_areas, aes(y = policy_areas, x = n))+
  geom_col()+
  geom_text(aes(label = n), hjust = 0)+
  labs(y = NULL, title = "Most common policy areas")

bottom_plot = ggplot(bottom_policy_areas, aes(y = policy_areas, x = n))+
  geom_col()+
  geom_text(aes(label = n), hjust = 0)+
  labs(y = NULL, title = "Least common policy areas")


(top_plot + bottom_plot)*
  scale_x_continuous(name = NULL, limits = c(0, 100), expand = expansion())*
  theme_minimal()
```
...
bills_became_law %>% 
  summarise(
    n_policy_areas = map_dbl(policy_areas, length)
  ) %>% 
  count(n_policy_areas)
```

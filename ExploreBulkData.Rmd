...
(sample_actions = sample_bill_df %>% 
  ...
  unnest(actions) %>% 
   select(action_type, actionCode, action_text, any_of(c("actionDate", "actionTime")), 
...
```{r}
(hr_legislative_subjects = hr_policy_areas %>% 
...
```{r}
(gov_ops_bills = hr_legislative_subjects %>% 
...
admin_law_bills %>% 
  ...
  mutate(action_ts = make_datetime(year = year(actionDate), 
                                   ...
                                   tz = "US/Eastern"), 
         .before = actionDate) %>% 
   arrange(bill_id, action_ts, action_type) %>% 
   # Number actions
  group_by(bill_id) %>% 

...
An overarching goal of the exploration is to create an app which provides people with a clearer view of who their representatives are and what they are doing.
I think the ideal solution combines Datamade's [beautifully simple representative finder](https://myreps.datamade.us/) and Project Vote Smart's [vast collection of data on legislator activities](https://justfacts.votesmart.org/).
...
```
View the bill in a table (Note I have a personal preference for the date format `dd/mm/yyyy` so that is what is used here)
...
select(sample_bill_df, 
  ...
  reactable(
    columns = list(
      ...
      createDate = colDef(name = "Created", width = 80, format = colFormat(date = T, locales = "en-GB")),
      latestAction_actionDate = colDef(name = "Date", width = 80, format = colFormat(date = T, locales = "en-GB")),
      latestAction_text = colDef(name = "Action", width = 320),
      version = colDef(show=F),
      updateDate = colDef(name = "Updated", width = 80, format = colFormat(date = T, locales = "en-GB")),
      introducedDate = colDef(name = "Introduced", width = 80, format = colFormat(date = T, locales = "en-GB"))
    ),
    columnGroups = list(
      colGroup(name = "Latest Action", columns = c("latestAction_actionDate", "latestAction_text"))
    )
  )
```
...
sample_actions %>% 
  ...
  scale_colour_manual(name = "Action", values = viz_colours[1:length(unique(sample_actions$action_type))])+
...
Can we scale this up to see what actions are taken on house bills on a timeline?

```{r}
# tic()
# (house_bills = future_map_dfr(flatten_chr(house_bill_files), extract_bill_status))
# toc()
# 
# saveRDS(house_bills, here("data", "cleaned", "BILLSTATUS_117_House.Rds"))

house_bills = readRDS(here("data", "cleaned", "BILLSTATUS_117_House.Rds"))
```

What does this data look like?

```{r}
glimpse(house_bills$hr)
```
...
What fields are available and when?
...
house_bills$hr %>% 
  ...
  count(action_type, actionCode, sort = T)
```


First, we need to unnest the actions data and see what action-level information looks like.
...
```{r}
(actions_dedupe = actions_unnested %>% 
  filter(actionCode != "Intro-H"|is.na(actionCode)) %>% 
   arrange(bill_id, action_type))
```
```{r}
...
(house_bill_actions = map(house_bills, select, where(negate(is_list)), starts_with("action")) %>% 
   # Unnest actions
  map_dfr(unnest, actions) %>% 
   distinct(bill_id, title, introducedDate, 
           actionDate, actionTime, action_source_name, action_type, action_text, action_committee_name, .keep_all = T) %>% 
   # Order bills and actions
...
```{r}
house_bill_actions %>% 
  filter(billType == "HR") %>% View()
  # select(bill_id, title, became_law, action_source_name, action_ts, action_type, actionCode, action_text) %>% 
  mokeR::reactable(
    resizable = T, 
    ...
    columns = list(
      title = colDef(aggregate = "unique", width = 200),
      action_ts = colDef(),
      createDate = colDef(aggregate = "unique", format = colFormat(date = T)),
      updateDate = colDef(aggregate = "unique", format = colFormat(date = T)),
      action_text = colDef(width = 980),
      became_law = colDef(aggregate = "unique")
      )
  )
```

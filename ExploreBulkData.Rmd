...
```{r sample-bill-rt}
sample_col_group = if("latestAction_actionDate" %in% colnames(sample_bill_df)){
  list(
...
select(sample_bill_df, 
       bill_id, title, where(negate(is_list)), introducedDate, createDate,
  ...
  reactable(
    columns = list(
      ...
      createDate = colDef(show = F,name = "Created", width = 100,
                          format = colFormat(date = T, locales = "en-GB")),
      latestAction_actionDate = colDef(name = "Date", width = 100,
                                       format = colFormat(date = T, locales = "en-GB")),
      latestAction_text = colDef(name = "Action", maxWidth = 360),
      version = colDef(show=F),
      updateDate = colDef(show = F,name = "Updated", width = 100,
                          format = colFormat(date = T, locales = "en-GB")),
      introducedDate = colDef(name = "Introduced", width = 104, 
                              format = colFormat(date = T, locales = "en-GB"))
    ),
...
(sample_actions = sample_bill_df %>% 
  ...
  unnest(actions) %>% 
   select(bill_id, 
          action_type, actionCode, action_text, 
          any_of(c("actionDate", "actionTime")), 
...
sample_actions %>% 
  ...
  select(bill_id, title, action_type, actionCode, actionDate, action_text, action_source_name, action_committee_name)
```

# Create House bills dataset

```{r sample-hr-bills, eval=F, include=F}
# Test for sample
# Extract bill data for a sample of house bills
hr_sample = sample(house_bill_files$hr, 1000)
```

```{r extract-sample-bills, eval=F, include=F}
...
```{r sample-hr-glimpse, eval=F, include=F}
glimpse(hr_bills)
```
...
```{r extract-all-bills, eval=F, echo = T, cache=T}
...
To get an accurate timeline of the actions taken on each bill, we will want to look at the `actionDate`, `actionTime` (for Floor actions), `action_type`, `actionCode`, and `action_text`.
...
gov_ops_bills %>% 
  ...
  filter(n > 100) %>% 
  mutate(active = interval(start = introducedDate, end = latestAction_actionDate)) %>% 
  # pivot_longer(
  #   cols = ends_with("Date"), 
  #   names_to = "type", 
  #   values_to = "time", 

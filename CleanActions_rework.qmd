...
`unnest()` actions from the bill data to see action-level information.
...
`action_source_name` indicates the source of the action data and is useful for understanding how the data is constructed.
...
`action_type` is the field that aligns most closely with these steps.
The different values of `action_type`, where `n` is the number of actions:
...
(action_type_summary = actions_unnested %>% 
  ...
  group_by(action_type, total_bills) %>% 
...
Note there are cases where both `action_code` and `action_desc` are missing and cases where `action_desc` is missing while `action_code` is present.

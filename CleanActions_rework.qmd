...
First, let's have a look at the data. `unnest()` actions from the bill data to see action-level information.
...
(actions_unnested = select(all_bills, 
                           bill_id, title, type, actions) %>% 
   # Categorize type of legislation
  mutate(type_cat = case_when(type %in% c("HR", "S") ~ "Bill",
    ...
    # Unnest actions
  unnest(actions, names_sep = "_") %>% 
   # The format changes now require using names_sep
   # this means action columns have redundant prefix
   rename_with(.cols = matches("action_"), str_remove, "action_")) %>% 
  # After assignment: Subset the data
  head(20)

```

`glimpse()` provides the dimensions of the data, the data types and the first few elements of each column.

```{r}
#| label: glimpse-actions
glimpse(actions_unnested)
```

The data contains `r scales::comma(nrow(actions_unnested))` actions on `r scales::comma(n_distinct(actions_unnested$bill_id))` different bills.

Action data can include:
...
-   a specific code for the action

-   the committee related to the action

Naturally, each field is not always populated. How much data do we have for each column?

```{r}
actions_unnested %>% 
  # Select action columns
  select(starts_with("actions")) %>% 
  # Calculate # of data points present over # of rows
  summarise(across(everything(), ~sum(!is.na(.))/n())) %>% 
  # pivot
  pivot_longer(everything(), names_to = "column", values_to = "pct_present")
```

Let's look at some of the key columns in detail.



## Action Source

`actions_source_system_name` indicates the source of the action data and is useful for understanding how the data is constructed. What proportion of the data does each source type account for?

```{r}
actions_unnested %>% 
  add_tally() %>% 
  group_by(actions_source_system_name) %>% 
  summarise(
    actions = n(),
...
- Library of Congress accounts for a little under half the actions 

- House floor actions account for just under a third of actions

Which types of actions does each source collect?

```{r}
#| label: source-by-type
action_source_by_type = actions_unnested %>%
    # Count actions in each source
    add_count(actions_source_system_name, name = "source_actions") %>%
    group_by(actions_source_system_name, actions_type) %>%
    summarise(
...
action_source_by_type %>% 
  tidyr::complete(actions_source_system_name, actions_type) |> 
  pivot_wider(id_cols = c(actions_type),
              names_from = actions_source_system_name,
              values_from = c(actions))
...
To clean the data, and to break up the work, it helps to think about the different steps of the legislative process which, [according to congress.gov](https://www.congress.gov/legislative-process) are:
...
`actions_type` is the field that aligns most closely with these steps. The different values of `actions_type`, where `n` is the number of actions:

```{r}
count(actions_unnested, actions_type)
```

How often does each action type occur on a bill?

```{r}
(actions_type_summary = actions_unnested %>% 
  mutate(total_bills = n_distinct(bill_id)) %>% 
  group_by(actions_type, total_bills) %>% 
...
Bills are always introduced, but only 38% received any attention in committees, \~10% went to the floor, and 1.3% became law.

```{r}
(bills_became_law = all_bills %>% 
  filter(map_lgl(actions, ~ "BecameLaw" %in% .$type))) %>% 
  glimpse()
```

```{r}
skim(bills_became_law)
```

```{r}
bills_became_law %>% 
  select(bill_id, type, title, summaries) %>% 
  unnest(summaries, names_sep = "_")
```

```{r}
bills_became_law %>% 
  unnest(titles, names_sep = "_")
```

```{r}
bills_became_law %>% 
  unnest(text_versions, names_sep = "_")
```
...
```{r}
bills_became_law %>% 
  unnest(sponsors, names_sep = "_")
```

```{r}
bills_became_law %>% 
  unnest(cosponsors, names_sep = "_")
```

## Action Codes

`action_code` is a more fine-grain category than `actions_type` but is often missing. govinfo provides a table of action codes in the [Bill Status User Guide](https://github.com/usgpo/bill-status/blob/main/BILLSTATUS-XML_User_User-Guide.md#3-action-code-element-possible-values)and describes the issues with the codes too:
...
>
> *\
> The original code set can be found at <http://www.loc.gov/pictures/resource/ppmsca.33996/>.*
...
(actions_joined = actions_unnested %>% 
  ...
  left_join(action_codes, by = c("actions_code" = "action_code"))) %>% 
  glimpse()
```

How often are action codes anddescriptions present?
...
actions_joined %>% 
  summarise(
    actions = n(),
    code_present = sum(!is.na(actions_code)),
    code_desc_present = sum(!is.na(action_desc))
  )
```

Check if cases exist where a description is present without an action code?

```{r}
actions_joined %>% 
  filter(is.na(actions_code)) %>% 
  count(action_desc)
# Should return missing action desc and the number of actions without action codes
```

Note there are cases where both `actions_code` and `action_desc` are missing and cases where `action_desc` is missing while `actions_code` is present.

```{r}
# Count actions with codes and descriptions by bill and action type
(actions_codes_summary_stats = actions_joined %>% 
  # Make missing action types explicit
  mutate(actions_type = fct_explicit_na(actions_type)) %>% 
  # Count coded actions by chamber and action type
  group_by(actions_type) %>% 
  summarise(
    actions = n(),
    code_present = sum(!is.na(actions_code)),
    ...
    # code_no_desc = sum(!is.na(actions_code)&is.na(action_desc)),
    # code_no_desc_pct = code_no_desc/actions,
    .groups = "drop"
  )) %>% 
  mutate(across(ends_with("_pct"), scales::percent, accuracy = .1))

```

So there is some work to do for Intro/Referral, Committee, and Floor actions to develop comprehensive coding for each type of action (except perhaps `BecameLaw`). This means identifying actions missing action code or code description and categorizing them using the text of the action, the source name, date/time, and related actions as context.

There are a number of cases where actions are duplicated in the data which need to be reconciled in order to make sense of the legislative timeline. Below are a list of the potential duplicates I have found:
...
How are actions distributed across bills?
...
count(actions_unnested, bill_id) %>% 
  ...
  labs(x = "Actions", y = "Bills", 
       title = "How much action does a bill get?",
       subtitle = "Distribution of congressional actions recorded on bills")+
  theme_moke(base_size = 11, plot_margin = margin(0,0,0,0))+
...
```{r}
# Count actions for each bill
count(actions_unnested, type_cat, bill_id, name = "actions") %>% 
  # Group by bill type
  group_by(type_cat) %>% 
  # summary stats
  summarise(
...
The main components of this data cleaning are:

-   to categorize actions which are missing an action code or description, and

-   to remove duplicate actions

# Categorize Actions

## Intro/Referral actions
...
intro_actions = actions_joined %>% 
  filter(actions_type == "IntroReferral")
```
...
Action codes are very helpful because they allow us to differentiate between the various actions of the same general type. Some questions to answer about the action codes:

1.  Which actions have codes?
2.  How many actions are missing codes?
3.  How many actions have codes but are missing code descriptions?
4.  For actions with a code but without a description, can we infer a description for the action?
5.  For actions without a code, can we infer a description for the action similar to action codes?
6.  Do we have duplicate actions?

What codes do we have?

```{r}
count(intro_actions, actions_code, action_desc, sort = T)
```

How many actions are coded?

```{r}
nrow(intro_actions[!is.na(intro_actions$actions_code), ])
```

How many actions are coded but missing descriptions?

```{r}
nrow(intro_actions[!is.na(intro_actions$actions_code) & is.na(intro_actions$action_desc), ])
```

### Missing description

Can we come up with a description for action codes based on the text of their actions? Use `action_text` to get further information on action codes without descriptions.

```{r}
intro_actions %>% 
  # Filter actions with action codes but no action description
  filter(!is.na(actions_code), is.na(action_desc)) %>% 
  # Arrange data to compare text for action codes
  select(actions_code, actions_text, bill_id, actions_date, everything()) %>% 
  arrange(actions_code, bill_id) %>% 
  group_split(actions_code)# %>% 
...
(intro_actions_codes = tribble(
  ~actions_code, ~actions_code_desc,
...
For actions without a code, can we can we infer a description for the action similar to action codes? Below I count the instances of `action_text` to see which occur most often.

```{r}
(intro_actions_missing_code = intro_actions %>% 
  # Filter actions with action codes but no action description
  filter(is.na(actions_code))) %>% 
  count(actions_text, sort = T)
```
...
intro_actions_missing_code %>% 
  count(actions_source_system_name, sort = T)
```
...
intro_actions_missing_code %>%
  ...
  mutate(action_desc = case_when(
    str_detect(actions_text, "[Rr]eferred to the (Select\\s)?Committee") ~ "Referred to Committee",
    str_detect(actions_text, "Received in the Senate") ~ "Received in the Senate"
  )) %>% 
  count(action_desc, sort = T)
```

Join our action codes to intro actions and fill in missing descriptions using the above regex.

```{r}
(intro_actions_coded = intro_actions %>%
  # Join action codes
  left_join(intro_actions_codes, by = "actions_code") %>% 
  # Assign actions to category based on their text when missing action code
  mutate(actions_code_desc = case_when(
    !is.na(actions_code_desc) ~ actions_code_desc,
    str_detect(actions_text, "[Rr]eferred to the (Select\\s)?Committee") ~ "Referred to Committee",
    str_detect(actions_text, "Received in the Senate") ~ "Received in the Senate"
  ))) %>% 
  count(actions_code, actions_code_desc)
```
...
(actions_intro_coded = actions_intro_dedupe %>% 
  ...
  mutate(actions_cat = case_when(
    !is.na(action_desc) ~ action_desc,
    # Specify coded actions
    actions_code == "B00100" ~ "Sponsor introductory remarks",
    actions_code == "H11100" ~ "Referred to committee",
    actions_code == "H11210" ~ "House committee granted an extension for further consideration",
    # Specify uncoded actions
    str_detect(actions_text, "Referred to the Committee") ~ "Referred to committee",
    str_detect(actions_text, "^Read (twice|the second time) and referred") ~ "Read twice and referred to committee",
    str_detect(actions_text, "Received in the Senate and Read twice and referred") ~ "Received in the Senate, read twice, and referred to committee",
    str_detect(actions_text, "Received in the Senate and referred") ~ "Received in the Senate and referred to committee",
    actions_type == "IntroReferral" ~ actions_text
  ))) %>% 
  count(actions_cat, sort = T)
```
...
```{r}
count(committee_actions, actions_code, action_desc, sort = T)
```

How many actions are coded?

```{r}
nrow(committee_actions[!is.na(committee_actions$actions_code),])
```

How many actions are coded but missing descriptions?

```{r}
nrow(committee_actions[!is.na(committee_actions$actions_code)&is.na(committee_actions$action_desc),])
```
...
```{r}
committee_actions %>% 
  filter(actions_code %in% c("H12200", "H12100")) %>% 
  add_count(bill_id) %>% 
  count(n)
```
...
```{r}
(committee_actions_codes = tribble(
  ~actions_code, ~actions_code_desc,
...
committee_actions_joined = committee_actions %>% 
  left_join(committee_actions_codes, by = "actions_code")
```
...
(committee_actions_coded = committee_actions_joined %>% 
  ...
  # filter(is.na(actions_code_desc)) %>% 
  # select(action_text, everything()) %>% View("Missing code desc")
  count(actions_code_desc, sort = T)
...
committee_actions %>% 
  ...
  filter(actions_code_desc == "Referred to subcommittee") %>% 
...
committee_actions_coded %>% 
  add_count(bill_id) %>% 
  filter(n == max(n)) %>% 
  arrange(actions_date) %>% View("Example committee actions bill")
```
...
```{r}
count(floor_actions, actions_code, action_desc, sort = T)
```

How many actions are coded?

```{r}
nrow(floor_actions[!is.na(floor_actions$actions_code),])
```

How many actions are coded but missing descriptions?

```{r}
nrow(floor_actions[!is.na(floor_actions$actions_code)&is.na(floor_actions$action_desc),])
```
...
floor_actions %>% 
  # Filter actions with action codes but no action description
  filter(!is.na(actions_code), is.na(action_desc)) %>% 
  count(actions_code, actions_text) %>% 
  arrange(actions_code, desc(n))# %>% 
...
```{r}
floor_actions_codes = tribble(
  ~actions_code, ~action_desc,
...
```{r}
floor_ambiguous_actions_codes = c("H1B000", "H12700", "H30000", "H30200", "H36600", "H36610", "H8D000")

floor_actions %>% 
  filter(actions_code %in% floor_ambiguous_actions_codes) %>% 
  select(actions_text, actions_code, everything()) %>% 
  group_split(actions_code)
```
...
```{r}
discharge_actions %>% 
  count(actions_code, action_desc, sort = T)
```
...
discharge_actions %>% 
  add_count(actions_code, name = "actions_code_count") %>% 
  # Select distinct action codes and their descriptions
  distinct(actions_code, actions_code_count,
           action_desc, actions_text_trunc = str_trunc(actions_text, width = 60),
           actions_text) %>%
  # sort by frequency
  arrange(desc(actions_code_count), actions_code) %>% 
  head()# %>% 
  # View("Discharge Action Codes")
...
actions_joined %>% 
  filter(actions_type == "BecameLaw") %>% 
  count(actions_source_system_name, actions_code, sort = T)
...
actions_joined %>% 
  filter(actions_type == "(Missing Action Type)") %>% 
  count(actions_source_system_name, actions_code) %>% 
  arrange(actions_code, desc(n))
...
actions_joined %>% 
  ...
  add_count(actions_code, name = "actions_code_count") %>% 
  # Select distinct action codes and their descriptions
  distinct(actions_source_system_name, actions_code, actions_code_count,
           action_desc, actions_text_trunc = str_trunc(actions_text, width = 60),
           actions_text) %>%
  # sort by frequency
  arrange(desc(actions_code_count), actions_code) %>% 
...
```{r}
(intro_dupes = unnest(intro_actions_coded, actions_committees, names_sep = "_", keep_empty = T) |> #glimpse()
   get_dupes(bill_id, actions_date, 
             # actions_source_system_name, actions_source_system_code, actions_code, 
             actions_text, actions_type,
             actions_committees_systemCode,
             actions_committees_name))
```

```{r}
intro_dupes %>% 
  count(actions_code, wt = dupe_count, sort = T)
```

The `actions_code`s **1000** and **Intro-H** seem to be duplicates. Let's look at them.

::: callout-note
### Assumption

A bill being received in the Senate is different than being introduced in the Senate.\
If this is incorrect, we may need to deduplicate those too.
:::
...
(compare_intro_actions = intro_actions_coded %>% 
  filter(actions_code %in% actions_to_compare)) %>% 
  glimpse(width = 80)
```
...
compare_intro_actions %>% 
  count(bill_id, wt = n_distinct(actions_code), name = "code_count") %>% 
...
compare_intro_actions %>% 
  ...
  count(actions_code, wt = n_distinct(bill_id), name = "bills")
```
...
compare_intro_actions %>% 
  ...
  mutate(actions_present = case_when(
    all(actions_to_compare %in% actions_code) ~ "Both",
    "1000" %in% actions_code ~ "1000 only",
    "Intro-H" %in% actions_code ~ "Intro-H only"
  )) %>% 
  ungroup() %>% 
...
compare_intro_actions %>% 
  ...
  filter(n == 2) %>% 
  arrange(bill_id, actions_code) %>% 
  # Count the number of different values in non-code columns
  group_by(bill_id) %>% 
  summarise(across(c(-actions_code, -action_desc), n_distinct), 
            .groups = "drop") %>% 
...
compare_intro_actions %>% 
  ...
  filter(replace_na(actions_code != "Intro-H", T)) %>% 
  pull(bill_id) %>% 
  n_distinct()
```
...
(intro_actions_dedupe = intro_actions_coded %>% 
  # Remove Intro-H action codes (keep NAs)
  filter(replace_na(actions_code != "Intro-H", T)) %>% 
  arrange(bill_id, actions_date, actions_time)) %>% 
  glimpse(width = 80)
```
...
```{r}
(committee_dupes = unnest(committee_actions_coded, actions_committees, names_sep = "_", keep_empty = T) |> 
   get_dupes(actions_text, bill_id,actions_date, actions_time,
             actions_committees_systemCode,
             actions_committees_name))
```
...
committee_dupes %>% 
  count(actions_code, wt = dupe_count, sort = T) %>% 
  filter(actions_code %in% c("H12200", "H12100", "H12210")) %>% 
  summarise(sum(n))
```
...
committee_actions %>% 
  group_by(bill_id) %>% 
  filter("H12100" %in% actions_code) %>% 
  select(action_text, actions_code, everything())# %>% 
  # View("H12100 bills")
```
...
committee_actions_coded %>% 
  group_by(bill_id) %>% 
  filter("14000" %in% actions_code) %>% 
  ungroup() %>% 
  mutate(filter_cond = replace_na(!(actions_code %in% c("5000", "14000")), T)) %>% 
  select(action_text, actions_code, filter_cond, everything()) %>% 
  View("14000 actions")
```
...
(committee_actions_dedupe = committee_actions_coded %>% 
  ...
  filter(replace_na(!(actions_code %in% c("5000", "14000")), T)) %>% 
  # filter(replace_na(!(actions_code == "5000"), T),
  #        actions_code_desc != "Original measure reported to Senate") %>% 
...
Because time information is missing and often actions of different types occur on the same day, we need to make use `actions_type` to identify the order of actions.
...
unclear_action_example %>% 
  select(action_date, action_time, actions_type, action_text) %>% 
...
(actions_coded = actions_unnested %>% 
    ...
    mutate(action_ts = make_datetime(year = year(action_date), 
           ...
           actions_type_fct = fct_explicit_na(
             factor(actions_type, 
    ...
    relocate(action_ts, actions_type, action_text, 
                    actions_code, actions_type_fct,
                    actions_source_system_name, action_source_code,
...
actions_coded %>%
  count(type, actions_type) %>% 
  arrange(type, desc(n)) %>% 
  pivot_wider(names_from = actions_type, values_from = n, values_fill = 0)
```
...
actions_coded %>%
  ...
  ungroup() %>%
  # Plot
  ggplot(., aes(y = type, fill = fct_rev(actions_type)))+
  geom_col(aes(x = n), 
  ...
  scale_fill_manual(name = "Action\nType", 
                    values = viz_colours[length(unique(actions_unnested$actions_type)):1],
...
actions_unnested %>% 
  ...
  ungroup() %>% 
  arrange(mean_num) %>% 
  mutate(actions_type = fct_inorder(factor(actions_type))) %>% 
  ggplot(., aes(x = actions_type, y = action_number))+
  geom_boxplot()+
...
sample_bill_actions %>% 
  ...
  geom_point(aes(colour = actions_type, size = ..count..), stat = "count")+

...
```{r}
summary(actions_unnested)
```


The data contains `r scales::comma(nrow(actions_unnested))` actions on `r scales::comma(n_distinct(actions_unnested$bill_id))` different bills. How often do we have data for each column?

...
In order to understand how legislation moves through congress, the chronological order of actions is important to know.
  ...
  summarise(
    ...
    across(c(actionDate, actionTime), 
           list(n = ~sum(is.na(.), na.rm = T), 
                pct = ~sum(is.na(.), na.rm = T)/n_actions))
  ) %>% 
...
Because actionsThe reason for making this assumption is

...
First, let's `unnest()` the actions data and see what action-level information looks like.
...
actions_unnested %>% 
  ...
  select(bill_id, title, action_ts, 
         action_type, actionCode,
         action_text,
         action_source_name, action_committee_name)
```
...
```{r }
#| label:  action-codes
(actions_joined = actions_coded %>% 
  left_join(action_codes, by = c("actionCode" = "Code"))) %>% 
  select(bill_id, action_ts, action_type, actionCode, Action) %>% 

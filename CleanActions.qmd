...
First, `unnest()` the actions from the bills to see action-level information.
...
There are a few columns which are helpful for identifying which of these steps an action belongs to. `action_type` aligns most closely with these steps, while `action_code` is a more fine-grain category but is more often missing. `action_source_name` indicates the source of the action data and is useful for understanding how the data is constructed.
...
How many bills does each type of action occur on? This is a different question than the counts above because there can be multiple of the same action type on a bill.
...
From this we can see that bills were always introduced, but only 38% received any attention in committees, \~10% went to the floor, and 1.3% became law.
...
Note there are two types of issues here:

1.  Missing an action code
2.  Having an action code but missing a code description
...
action_codes_summary_stats %>% 
  ...
  labs(title = "How often are action code and code description information available?",
       subtitle = "Action code and description presence, by action type",
...
In cases where the action does not have a code, can we come up with a category with similar precision to an action code based on the action text?

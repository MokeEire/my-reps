...
So the vast majority of bills have 5 or fewer actions recorded on them. It seems like there are slightly more actions on concurrent resolutions, but this observation may change after cleaning the data.
...
There are a few different columns in the data we can use to categorise actions:
...
None of these directly segment actions into the steps mentioned above but `action_type` is the most closely aligned, while `action_code` is useful for distinguishing actions of the same type but as we'll see it is not perfect. `action_source_name` seems to indicate the source of the action data and is useful for understanding how the data is constructed.

Below are the counts of each variable.
...
How often does each type of action occur on bills?
...
Bills are always introduced, but only 37% receive any attention in committees.
...
actions_joined %>% 
  summarise(
    ...
    desc_present = sum(!is.na(Action)),
    code_no_desc = sum(!is.na(action_code)&is.na(Action))
  )
```
...
2.  Missing a code description, but having action codes

Does action code presence vary by the type of action?
...
How do action codes vary by bill type and action source?
...
What codes are used for Intro/Referral actions and which of these codes have descriptions?

```{r}
count(intro_actions, action_code, Action, sort = T)
```

What sources are the action codes coming from?

```{r}
count(intro_actions, action_code, action_source_name, sort = T)
```

So all of the missing action codes are coming from the Senate.
...
How many of these two identical actions do bills have?
...
How many bills does each code occur on?

```{r}
compare_intro_actions %>% 
  # Count bills action codes occur on
  count(action_code, wt = n_distinct(bill_id))
```

Does one code occur in every case?
...
When bills have more than one action, is all the information identical other than the action code? We'll check this by getting the different distinct combinations of variables excluding action code (and its related description we joined earlier).

If we get the same number of rows as there are bills, the information is identical.
...
(intro_actions_dedupe = intro_actions %>% 
...
#### Incomplete Action Coding

Of the de-duplicated Intro/Referral actions, how many have action codes and descriptions?

```{r}
intro_actions_dedupe %>% 
  left_join(intro_action_codes, by = "action_code") %>% 
  summarise(
    n = n(),
    across(c(action_code, Action, action_desc), ~sum(!is.na(.)))
  )
```

Let's use the action text field to categorize them.
...
These seem to be senate committee referral actions, can we verify with the source name?
...
intro_actions_dedupe %>% 
  # Join action codes
  left_join(intro_action_codes, by = "action_code") %>% 
  # Keep cases without any action codes
  filter(is.na(action_code)|is.na(action_desc)) %>%
...
```{r}
(intro_actions_dedupe_coded = intro_actions_dedupe %>%
  # Join action codes
  left_join(intro_action_codes, by = "action_code") %>% 
...
What about the actions which don't have a code? Let's see the action text and categorize with some regex.
...
```{r}
(committee_actions_coded = committee_actions %>% 
  ...
  mutate(action_desc = case_when(
    ...
    str_detect(action_text, 
               "Committee Consideration and Mark\\-up") ~ "Committee Mark-up Session",
    str_detect(action_text, 
...
committee_actions_coded %>% 
  add_count(action_desc, name = "cat_n") %>% 
  count(action_desc, action_text, cat_n) %>%
  arrange(desc(cat_n), action_desc, desc(n)) %>% 
  View("Category elements")
```

```{r}
#| label: view-committee-actions-by-date
#| eval: false
committee_actions_coded %>% 
  select(bill_id, title, action_date, action_desc, action_text, action_committee_name, action_source_name) %>% 
  arrange(bill_id, action_date) %>% View()
```

```{r}
#| label: view-many-committee-actions-by-date
#| eval: false
committee_actions_coded %>% 
  add_count(bill_id) %>% 
  filter(n > 7) %>% 
...
```{r}
floor_ambiguous_action_codes = c("H1B000", "H30000", "H30200", "H36600", "H36610")

...
The iterative process that helped me come up with these categories looked something like this:
...
```{r}
(committee_dupes = janitor::get_dupes(committee_actions_coded,
...
```{r}
committee_actions_coded %>% 
  filter(action_code %in% c("H12200", "H12100", "H12210", "5000"))
```
...
Yep we also have duplicates of this action. So `H12100`, `H12200`, and `H12210` are all duplicates of `5000`. Let's keep the `H12-` action codes because they delineate the different actions.

What about the Original measure reported to Senate action without an action code that appears to be a duplicate of code `14000`

```{r}
committee_actions_coded %>% 
  filter(action_code_desc == "Original measure reported to Senate")
```

So we can remove `5000` actions without losing any information or bills. This reduces the size of the actions data by thousands of rows.
...
floor_actions %>% 
  ...
  arrange(action_code, bill_id) %>% 
  group_split(action_code) %>% 
  map(count, action_code, action_text, sort = T)

...
Using the `clean_data.R` script, I parsed govinfo's house and senate bill data from the current (117th) congress. This document looks at the actions on these bills and outlines the steps required to clean the actions data. This includes:
...
-   the date of the action, in some cases the time
...
The data contains `r scales::comma(nrow(actions_unnested))` actions from `r scales::comma(n_distinct(actions_unnested$bill_id))` bills.
...
There are a few different columns we can use to categorise actions:
...
As described by Congress' website, the legislative process most generally maps to `action_type`. `action_code` is useful for distinguishing between actions of the same type, but as we'll see it is not perfect. `action_source_name` seems to indicate the source of the action data and is useful for understanding how the data is constructed. Below are the counts of each variable.

```{r}
#| layout-nrow: 1
count(actions_unnested, action_type)

count(actions_unnested, action_code)

count(actions_unnested, action_source_name)
```

What proportion of bills contain each type of action?

```{r}
# add action count for each bill
add_count(actions_unnested, action_type, name = "actions") %>% 
  # tally total bills
  add_tally(wt = n_distinct(bill_id), name = "total_bills") %>% 
  # Count bills by action count
  count(action_type, actions, total_bills, 
        wt = n_distinct(bill_id), name = "bills") %>% 
  mutate(bill_prop = scales::percent(bills/total_bills)) %>% 
  select(-total_bills)
```
...
I'll need to develop my own set of categories to break down the different kinds of committee and floor actions. To start, I join these codes using a two column csv file to see where we have action code information. This looks like:
...
1.  Missing action codes
2.  Having an action code, but missing a description for that code
...
actions_joined %>% 
  summarise(
    actions = n(),
    actions_codes = sum(!is.na(action_code)),
    actions_codes_desc = sum(!is.na(Action)),
    actions_codes_no_desc = sum(!is.na(action_code)&is.na(Action))
  )
...
action_codes_summary_stats = actions_joined %>% 
  ...
  group_by(bill_type_cat, action_type) %>% 
  summarise(
    actions = n(),
    actions_coded = sum(!is.na(action_code)),
    actions_coded_pct = actions_coded/actions,
    actions_coded_desc = sum(!is.na(Action)),
    actions_coded_desc_pct = actions_coded_desc/actions,
    actions_coded_no_desc = sum(!is.na(action_code)&is.na(Action)),
    actions_coded_no_desc_pct = actions_coded_no_desc/actions,
    .groups = "drop"
  )
```
...
action_codes_summary_stats %>% 
  # Plot
  ggplot(., aes(y = fct_rev(action_type), group = bill_type_cat))+
  # All actions
  geom_col(aes(x = actions),
           position = position_dodge(width = 1),
           colour = my_col_pal[3], alpha = .85)+
  # Coded actions
  geom_col(aes(x = actions*actions_coded_pct, fill = bill_type_cat),
           colour = my_col_pal[3],
           position = position_dodge(width = 1),
           alpha = .95)+
  # Coded actions
  ggpattern::geom_col_pattern(aes(x = actions*actions_coded_no_desc_pct,
                                  pattern_fill = bill_type_cat),
  ...
  scale_fill_manual(name = NULL, values = viz_colours[1:4])+
  ggpattern::scale_pattern_fill_manual(name = NULL, values = viz_colours[1:4])+
  labs(title = "How often are action codes available?",
       subtitle = "Striped area indicates there is a code but no description",
...
Proportion of actions coded, grouped by different variables
...
actions_joined %>% 
  ...
  summarise(
    actions = n(),
    actions_coded = sum(!is.na(Action)),
    actions_coded_pct = scales::percent(actions_coded/actions),
    .groups = "drop"
  )
```
...
So there's a bit of work to do to develop comprehensive coding for each type of action (except perhaps `BecameLaw`). To do this, I will look at each action type and do two things:

-   Separate actions missing an action code or have an action code but do not have a description

The actions data is messy.

There are a number of cases where actions are duplicated in the data which need to reconcile in order to make sense of the legislative timeline. Are we able to identify duplicate actions using action codes?
...
(action_source_by_type = actions_joined %>%
    ...
    add_count(action_source_name, name = "source_actions") %>%
    group_by(action_source_name, source_actions, action_type) %>%
    summarise(
    bills = n_distinct(bill_id),
    actions = n(),
    # actions_pct = scales::percent(n()/unique(source_actions), accuracy = .1),
...
action_source_by_type %>%
  pivot_wider(id_cols = action_source_name,
              names_from = action_type,
              values_from = c(bills, actions, actions_pct),
              names_glue = "{action_type}_{.value}",
              values_fn = list(actions_pct = scales::percent),
              values_fill = list(bills = 0, actions = 0, actions_pct = "0%")) %>%
  select(action_source_name,
         starts_with("IntroReferral"), starts_with("Committee"),
          starts_with("Floor"), starts_with("Discharge"),
          starts_with("President"), starts_with("BecameLaw"), starts_with("NA")) %>%
  ...
  reactable(theme = moke_rt(),
            columns = list(
              "IntroReferral_actions" = colDef(name = "Actions", width = 80),
              "IntroReferral_actions_pct" = colDef(name = "%"),
              "IntroReferral_bills" = colDef(name = "Bills"),
              "Committee_actions" = colDef(name = "Actions"),
              "Committee_actions_pct" = colDef(name = "%"),
              "Committee_bills" = colDef(name = "Bills"),
              "Floor_actions" = colDef(name = "Actions"),
              "Floor_actions_pct" = colDef(name = "%"),
              "Floor_bills" = colDef(name = "Bills"),
              "Discharge_actions" = colDef(name = "Actions"),
              "Discharge_actions_pct" = colDef(name = "%"),
              "Discharge_bills" = colDef(name = "Bills"),
              "President_actions" = colDef(name = "Actions"),
              "President_actions_pct" = colDef(name = "%"),
              "President_bills" = colDef(name = "Bills"),
              "BecameLaw_actions" = colDef(name = "Actions"),
              "BecameLaw_actions_pct" = colDef(name = "%"),
              "BecameLaw_bills" = colDef(name = "Bills"),
              "NA_actions" = colDef(name = "Actions"),
              "NA_actions_pct" = colDef(name = "%"),
              "NA_bills" = colDef(name = "Bills")
            ),
            columnGroups = list(
              colGroup(name = "Intro/Referral",
                       columns = c("IntroReferral_actions",
                                  "IntroReferral_actions_pct",
                                  "IntroReferral_bills")),
              colGroup(name = "Committee",
                        columns = c("Committee_actions", "Committee_actions_pct",
                        "Committee_bills")),
              colGroup(name = "Floor",
                        columns = c("Floor_actions", "Floor_actions_pct",
                        "Floor_bills")),
              colGroup(name = "Discharge",
                        columns = c("Discharge_actions", "Discharge_actions_pct",
                        "Discharge_bills")),
              colGroup(name = "President",
                        columns = c("President_actions", "President_actions_pct",
                        "President_bills")),
              colGroup(name = "Became Law",
                        columns = c("BecameLaw_actions", "BecameLaw_actions_pct",
                        "BecameLaw_bills")),
              colGroup(name = "Missing",
                       columns = c("NA_actions", "NA_actions_pct", "NA_bills"))
))
```
...
Committee and Discharge actions are not well coded, but the majority Intro/Referrals and Floor actions are coded.
...
What codes are used for Intro/Referral actions?

```{r}
count(intro_actions, action_code, sort = T)
```

Which source are Intro/Referral actions coming from?
...
What action codes do we have for Intro/Referral actions and what source system are they coming from?

```{r}
intro_actions %>% 
  count(action_code, Action, action_source_name, sort = T)
```

Look at the action texts each action code has.

```{r}
intro_actions %>% 
  # Add action code count
  add_count(action_code, name = "code_count") %>% 
  # Count the text associated with each action code
  count(action_code, code_count, action_text, name = "text_count") %>%
  # Sort by most frequent codes, then texts
  arrange(desc(code_count), action_code, desc(text_count))
```
...
(compare_intro_actions = intro_actions %>% 
...
What about actions without action codes or action labels?
...
```{r}
(intro_actions_uncoded = intro_actions %>% 
  # Remove Intro-H action codes (keep NAs)
  filter(replace_na(action_code != "Intro-H", T),
         is.na(Action)))
```

Are they all from a particular source e.g. Senate? Yep.

```{r}
intro_actions_uncoded %>% 
  count(action_source_name)
```

```{r}
intro_actions_uncoded %>% 
  count(action_code, action_text) %>% 
  View("intro action texts")
```
...
-   Read twice and referred to Committee X
...
(committee_actions_categorised = committee_actions %>% 
  ...
  mutate(committee_action_cat = case_when(
    ...
    # str_detect(action_text, 
    #            "Ordered to be reported with (an )?amendments?") ~ "Ordered to be reported with an amendment",
    str_detect(action_text,
...
Compare action texts in each category

```{r}
committee_actions_categorised %>% 
  add_count(committee_action_cat, name = "cat_n") %>% 
  count(committee_action_cat, action_text, cat_n) %>%
  arrange(desc(cat_n), committee_action_cat, desc(n)) %>% 
  View("Category elements")
```

```{r}
committee_actions_categorised %>% 
  select(bill_id, title, action_date, committee_action_cat, action_text, action_committee_name, action_source_name) %>% 
  arrange(bill_id, action_date) %>% View()
```
...
committee_actions_categorised %>% 
  ...
  filter(n > 7) %>% 
  select(bill_id, title, action_date, committee_action_cat, action_text, action_committee_name, action_source_name) %>% 
  arrange(bill_id, action_date) %>% 
  View("Bills with a lot of committee actions")
...
Combine these committee action categories with action codes

```{r}
committee_actions_categorised %>% 
  mutate(action_code_combined = coalesce(Action, committee_action_cat))
```

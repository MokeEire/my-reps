...
How often does each type of action occur on a bill?
...
add_count(actions_unnested, action_type, name = "actions") %>% 
  ...
  mutate(action_freq = scales::percent(bills/total_bills)) %>% 
  select(-total_bills)
```
...
How does action code presence vary by action type?
...
action_codes_summary_stats %>% 
  ...
  labs(title = "What actions are coded?",
       subtitle = "Action code availability, by action type",
...
actions_joined %>% 
  ...
  summarise(
    ...
    actions_coded_pct = scales::percent(coded/n()),
    .groups = "drop"
  )
```

So we have a good bit of work to do for Intro/Referral, Committee, and Floor actions. I am going to split the data by action type to break the task up.

So there's a bit of work to do on Committee and Floor actionsto develop comprehensive coding for each type of action (except perhaps `BecameLaw`). To do this, I will look at each action type and do two things:
...
Do bills typically have both of these actions?
...
Does one of the actions occur on every bill?

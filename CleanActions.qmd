...
count(actions_unnested, bill_id) %>% 
  ...
  labs(x = "Actions", y = "Bills", 
       title = "How many actions occur on each bill?",
...
As described by Congress' website, the legislative process most generally maps to `action_type`. `action_code` is useful for distinguishing between actions of the same type, but as we'll see it is not perfect. `action_source_name` seems to indicate the source of the particular action data and is useful mostly for understanding how the data is constructed.
...
On how many bills do each type of action occur?
...
I'll need to develop my own set of categories to break down the different kinds of committee and floor actions. To start, I join these codes using a two column csv file to see where we have action code information.

...
count(actions_unnested, origin_chamber, bill_id) %>% 
  ...
  theme(axis.line.y = element_blank(),
        legend.position = c(.925, .97))
```

What type of actions occur most often?

```{r}
# Count rows for each bill
add_count(actions_unnested, action_type, name = "actions") %>% 
  # Count bills by the number of actions
  count(action_type, actions, wt = n_distinct(bill_id), name = "bills" )
```
...
actions_joined %>% 
  ...
  geom_histogram(colour = my_col_pal[1], alpha = .9,
...
They're only coming from the Library of Congress. What proportion of the data are coming from which source?
...
actions_joined %>% 
  ...
  summarise(
    actions = n(),
    actions_pct = scales::percent(n()/unique(n), accuracy = .1),
    bills = n_distinct(bill_id),
    # bills_pct = bills/sum(bills)
  )
```

Library of Congress accounts for a little under half while House floor actions accounts for a little less than a third of all actions. What action types are contained in each source system?

...
Using the `clean_data.R` script, I have parsed govinfo's house and senate bill data from the 117th congress. In this document, I will be looking at the actions on these bills and cleaning them up. This includes:
...
What does the data look like? `glimpse()` gives us the dimensions of the data, the data types and the first few elements of each column.

```{r}
glimpse(actions_unnested, width = 80)
```

The data consists of `r scales::comma(nrow(actions_unnested))` actions from `r scales::comma(n_distinct(actions_unnested$bill_id))` bills. What's the distribution of actions per bill?
...
count(actions_unnested, origin_chamber, bill_id) %>% 
  ...
  labs(x = "Actions", y = "Bills", 
       title = "How much does congress interact with legislation?",
       subtitle = "Distribution of actions on bills in the House and Senate")+
  theme_moke(plots_pane = T)+
  theme(axis.line.y = element_blank(),
        legend.position = c(.9, .7))
```

Actions can fall into a few different categories like Intro/Referral, Committee, and Floor actions. How many actions of each type are in the data, and on how many bills do each type of action occur?
...
You might have noticed there is an `action_code` field in the data which sounds tremendously useful. govinfo provides a table of action codes in the [Bill Status User Guide](https://github.com/usgpo/bill-status/blob/main/BILLSTATUS-XML_User_User-Guide.md#3-action-code-element-possible-values), but unfortunately, as the Government Publishing Office describes, there are a number of issues:
...
But action codes will be vital for understanding how congress members interact with legislation, so I'll need to develop my own set of categories to break down the different kinds of committee and floor actions. To start, I join in these codes using a simple two column csv file to see where we have action code information.
...
actions_joined %>% 
  ...
  group_by(origin_chamber, action_type) %>% 
  summarise(
    actions = n(),
    actions_w_code = sum(!is.na(action_code)),
    actions_w_code_no_desc = sum(!is.na(Action)),
    actions_coded = actions_w_code/actions,
    actions_coded_no_desc = actions_w_code_no_desc/actions,
    .groups = "drop"
  ) %>% 
  # Plot
  ggplot(., aes(y = fct_rev(action_type), group = origin_chamber))+
  # All actions
  geom_col(aes(x = actions),
           position = position_dodge(width = 1),
           colour = my_col_pal[3], alpha = .85)+
  # Coded actions
  geom_col(aes(x = actions*actions_coded, fill = origin_chamber),
           colour = my_col_pal[3],
           position = position_dodge(width = 1),
           alpha = .95)+
  # Coded actions
  ggpattern::geom_col_pattern(aes(x = actions*actions_coded_no_desc,
                                  pattern_fill = origin_chamber),
  ...
  scale_fill_manual(name = NULL, values = viz_colours[1:2])+
  ggpattern::scale_pattern_fill_manual(name = NULL, values = viz_colours[1:2])+
  labs(title = "How often are action codes available?",
       ...
       y = NULL, x = "Actions")+
  facet_wrap(~origin_chamber, scales = "free_x")+
  theme_moke(plots_pane = T)+
  theme(legend.position = "none",
        panel.grid.major.y = element_blank(),
        panel.grid.major.x = element_line())

scales::show_col(my_col_pal)
```
...
-   Separate out actions which are either missing the action code or have an action code but do not have a description

```{r}
actions_joined %>% 
  group_by(origin_chamber) %>%
  # Count actions, actions w/code, and coded proportion
  summarise(
    actions = n(),
    actions_w_code = sum(!is.na(Action)),
    coded_prop = actions_w_code/actions,
    .groups = "drop"
  )
```
...
actions_joined %>% 
  ...
  # mutate(coded_prop = scales::percent(coded_prop)) %>%
  # count(origin_chamber, coded_prop, wt = n_distinct(bill_id))
  ggplot(., aes(x = coded_prop, fill = origin_chamber))+
  ...
  scale_fill_manual(name = NULL, values = viz_colours[1:2])+
  labs(title = "What proportion of a bill's actions are coded?",
